...
class FeeIndependentOfPayerGuardTests(unittest.TestCase):
    ...
    def test_fee_vector_independent_of_payer(self) -> None:
        ...
        action = AddLiquidity(pool_id="p", amount_a=3, amount_b=5)
        quote_a = quote_fee_for_action(engine, state, action, payer="payer-a")
        quote_b = quote_fee_for_action(engine, state, action, payer="payer-b")
